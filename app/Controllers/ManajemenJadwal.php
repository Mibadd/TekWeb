   ...
   public function update($id)
{
    ...
    // Update relasi suku cadang
    $builder = $this->db->table('jadwal_sukucadang');
    $builder->where('jadwal_id', $id)->delete(); // hapus yang lama
    ...
    public function edit($id)
{
    ...
    $sukuCadang = $sukuCadangModel->findAll();

    // Ambil suku cadang yang dipakai oleh jadwal ini
    $builder = $db->table('jadwal_sukucadang');
    $query = $builder->where('jadwal_id', $id)->get();
    $sukuCadangTerpakai = array_column($query->getResultArray(), 'sukucadang_id');

    if (!$jadwal) {
        return redirect()->to('manajemenjadwal')->with('error', 'Data tidak ditemukan.');
    }

    return view('admin/jadwal/editjadwal', [
        'jadwal' => $jadwal,
        'sukuCadang' => $sukuCadang,
        'sukuCadangTerpakai' => $sukuCadangTerpakai
    ]);
}
